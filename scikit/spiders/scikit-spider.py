...
class ScikitSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        item['funcDesc'] = section.xpath("dl/dd/p/text()").extract_first().replace('\n', ' ')
        item['notes'] = ' '.join([txt.strip().replace('\n', ' ') for txt in section.xpath("//p[text()='Notes']/following-sibling::p[not(@class='rubric')]//text()").extract()]) #returns text from the follwoing <p> tags as a single string, if the <p> tag is not associated with "rubric" class
        item['methods'] = []
            ...
            for method in response.xpath('//dl[@class="method"]'):
                ...
                for element in method.xpath("dt/descendant::text()[not(parent::a)]").extract():
                    if "\n" not in element and '[source]' not in element:
                        text = text + ''.join(element.replace('\n', ' '))
                method_item['methodName'] = text
                method_item['methodDesc'] = ' '.join([txt.strip().replace('\n', ' ') for txt in method.xpath("dd/p//text()").extract()])
                ...
                if innerMethodTable:
                    for row in innerMethodTable.xpath('tbody/tr[@class="field-odd field"] | tbody/tr[@class="field-even field"]'):
                        ...
                        if "Parameters" in fieldname.extract_first():
                            ...
                            method_item['methodParamsBody'] = ' '.join([txt.strip().replace('\n', ' ') for txt in row.xpath("td//text()").extract()])
                        
                        if "Returns" in fieldname.extract_first():
                            method_item['methodReturns'] = row.css("td p strong::text").extract()
                            method_item['methodReturnsBody'] = ' '.join([txt.strip().replace('\n', ' ') for txt in row.xpath("td//text()").extract()])
